 ...
 */
function newspack_blocks_get_social_icon_svg_map() {
	static $map = null;
	if ( null !== $map ) {
		return $map;
	}
 ...
 */
function newspack_blocks_get_fallback_social_svg( $service ) {
	$map = newspack_blocks_get_social_icon_svg_map();
...
function newspack_blocks_render_author_profile_social( $attributes, $content, $block ) {
	...
	if ( ! empty( $block->inner_blocks ) ) {
		return newspack_blocks_render_social_with_inner_blocks( $attributes, $block, $author, $icon_size );
	}

	// Legacy flat render: build social links from author data.
	return newspack_blocks_render_social_flat( $attributes, $block, $author, $icon_size );
}
 ...
 */
function newspack_blocks_render_social_with_inner_blocks( $attributes, $block, $author, $icon_size ) {
	$inner_content = '';
	...
	$all_services = newspack_blocks_get_author_available_services( $author );
	foreach ( $all_services as $service ) {
		if ( ! in_array( $service, $saved_services, true ) ) {
			// Render unsaved service as a fallback.
			$fallback_block = new WP_Block(
				[
					'blockName' => 'newspack-blocks/author-social-link',
 ...
 */
function newspack_blocks_get_author_available_services( $author ) {
	$services = [];
 ...
 * @return string Rendered HTML.
 */
function newspack_blocks_render_social_flat( $attributes, $block, $author, $icon_size ) {
	...
	foreach ( $social_links as $service => $social_data ) {
		...
		$svg = ! empty( $social_data['svg'] ) ? $social_data['svg'] : newspack_blocks_get_fallback_social_svg( $service );
