 ...
 */
function newspack_blocks_get_author_or_guest_author( $author_id ) {
	$author = false;
	...
	if ( class_exists( 'CoAuthors_Guest_Authors' ) ) {
		...
		if ( $author && isset( $author->ID ) ) {
			$author = [
				...
				'avatar' => function_exists( 'coauthors_get_avatar' ) ? coauthors_get_avatar( $author, 240 ) : false,
				'url'    => esc_urL(
					get_site_urL(
						null,
	...
	if ( ! $author ) {
		...
		if ( $author && isset( $author->data ) ) {
			$author = [
				...
				'avatar' => get_avatar( $author_id, 240 ),
...
function newspack_blocks_render_block_author_profile( $attributes ) {
	...
	$author = newspack_blocks_get_author_or_guest_author( intval( $attributes['authorId'] ) );
