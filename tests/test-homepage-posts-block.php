...
class HomepagePostsBlockTest extends WP_UnitTestCase_Blocks { // phpcs:ignore
	...
	/**
	 * `matchAllConditions` option, using taxonomies and authors.
	 */
	public function test_hpb_match_all_conditions_taxonomies_and_authors() {
		$cat_1_id            = wp_create_term( 'cat-1', 'category' )['term_id'];
		$post_with_cat_1_id  = self::create_post(
			[
				'post_category' => [ $cat_1_id ],
		...
		$author_1_id         = self::factory()->user->create( [ 'role' => 'editor' ] );
		$post_by_author_1_id = self::create_post(
			[
				'post_author' => $author_1_id,
			]
		);

		$query_with_author_and_category = self::query_from_attributes(
			[
				'categories' => [ $cat_1_id ],
				'authors'    => [ $author_1_id ],
			]
		...
		self::assertEquals(
			[],
			$query_with_author_and_category->posts,
			'No posts are returned since no posts by author-1 have category-1.'
		);
		$query_with_author_and_category_but_dont_match_all = self::query_from_attributes(
			[
				'categories'         => [ $cat_1_id ],
		...
		self::assertEquals(
			[ $post_with_cat_1_id, $post_by_author_1_id ],
			array_column( $query_with_author_and_category_but_dont_match_all->posts, 'ID' ),
			'Both posts are returned if not all conditions should be matched.'
		);
	}
